...
**実装作業は必ずサブエージェントが担当する。メインエージェントは直接コードを編集してはならない。**

- コード実装: `rust-implementation-specialist` サブエージェントを使用
- コード簡素化: `rust-simplification-specialist` サブエージェントを使用
- 要件定義・FPレビュー: `functional-programming-specialist` サブエージェントを使用
