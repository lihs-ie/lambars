...
mod tests {
    ...
    /// Concurrency test verifying that no changes are lost during RCU retries.
    ///
    /// # Test Strategy
    ///
    /// 1. Use a Barrier to guarantee synchronized start of all threads
    /// 2. Each thread executes multiple batch updates (to provoke retries)
    /// 3. Collect `TaskId`s generated by each thread (for post-verification)
    /// 4. After all threads complete, compare against `all_tasks()` `TaskId` set to detect losses
    ///
    /// # RCU Contention and Retries
    ///
    /// `ArcSwap::rcu` performs optimistic concurrency control using Compare-And-Swap (CAS).
    /// When multiple threads attempt updates simultaneously, some threads will retry
    /// due to CAS failures. This test verifies that no changes are lost during retries.
    ///
    /// # Verifying RCU Retries
    ///
    /// Run the test with `RUST_LOG=search_index_batch=trace` to see the
    /// `update_search_index_batch completed` log with details of each batch update.
    /// When contention occurs, `apply_changes_us` tends to be longer.
    ///
    /// ```bash
    ...
    ///
    /// Uses `std::thread::spawn` with OS threads because `std::sync::Barrier` is used.
    /// In a Tokio async context, `Barrier::wait()` would be a blocking call that causes
    /// deadlocks, so OS threads are used instead.
    #[rstest]
    #[tokio::test]
